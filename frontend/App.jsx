...
export default function App() {
  ...
  useEffect(() => {
    ...
    const LD1_JUSTIN_START = feetToFrontendPos(6, 9);
    const LD1_JUSTIN_GOAL = feetToFrontendPos(6, 19);
    const LD1_LOGAN_START = feetToFrontendPos(6, 5);
    const LD1_LOGAN_GOAL = feetToFrontendPos(6, 15);
    const LD1_TARGET_POS = feetToFrontendPos(38, 20);
    const LD1_BASE_SPEED = 80;  // cm per sec
    const LD1_ACCEL = 120;      // cm/s² random acceleration for drift
    ...
    function runLiveDemo1Script(s, dtSec) {
      ...
      if (script.phase === 0) {
        // Phase 0: Justin moves to (6,19) facing down — velocity + drift
        const dx = LD1_JUSTIN_GOAL.x - justin.position.x;
        const dy = LD1_JUSTIN_GOAL.y - justin.position.y;
        ...
        // Phase 2: Target appears — target 1 is spotted
        justin.position = { ...LD1_JUSTIN_GOAL };
        justin.facing = 0;
        logan.position = { ...LD1_LOGAN_START };
        logan.facing = Math.PI / 2;  // down
        ...
        // Phase 3: Logan moves from (6,5) to (6,15) after target spotted — velocity + drift
        justin.position = { ...LD1_JUSTIN_GOAL };
        justin.facing = 0;
        const dx = LD1_LOGAN_GOAL.x - logan.position.x;
        const dy = LD1_LOGAN_GOAL.y - logan.position.y;
        const dist = Math.hypot(dx, dy);
        if (dist < 5) {
          logan.position = { ...LD1_LOGAN_GOAL };
          script.loganVel = { vx: 0, vy: 0 };
          script.phase = 4;
        } else {
          const ux = dist > 0.1 ? dx / dist : 0, uy = dist > 0.1 ? dy / dist : 0;
        ...
        // Phase 4: Logan at (6,15), turn FOV from down to right, then steady
        justin.position = { ...LD1_JUSTIN_GOAL };
        justin.facing = 0;
        logan.position = { ...LD1_LOGAN_GOAL };
  ...
  const liveDemo1ScriptRef = useRef({ phase: 0, startTime: 0, justinVel: { vx: 0, vy: 0 }, loganVel: { vx: 0, vy: 0 } });

  const initLiveDemo1Script = useCallback(() => {
    const loganPos = feetToFrontendPos(6, 5);
    const justinPos = feetToFrontendPos(6, 9);
    const now = Date.now();
    stateRef.current.agents = [
      { id: "Logan", position: loganPos, vel: { vx: 0, vy: 0 }, facing: Math.PI / 2 },
      { id: "Justin", position: justinPos, vel: { vx: 0, vy: 0 }, facing: Math.PI / 2 },
    ];
